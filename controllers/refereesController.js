// import Referees from "../models/Referee.js";
...
async function collectData(browser, page, leechUrl) {
  ...
  try {
    ...
    try {
      await page.waitForSelector("//*[contains(@id, 'dtlHakemBilgi')]", {
        //find the cell that holds the details for the game and wait until it is visible
        // timeout: 2000,
        state: "visible",
      });
      let refName = await page
        .locator("span[id$='dtlHakemBilgi_Label1']")
        .elementHandles();
      let refLicence = await page
        .locator("//span[contains(@id,'dtlHakemBilgi_Label3')]")
        .elementHandles();
      let refClass = await page
        .locator("//span[contains(@id,'dtlHakemBilgi_Label4')]")
        .elementHandles();
      let refReg = await page
        .locator("//span[contains(@id,'dtlHakemBilgi_Label6')]")
        .elementHandles();
      ...
      while (true) {
        ...
        let check = await page
          .locator("//tfoot/tr/td/a[last()]")
          .allTextContents();
        console.log(check);
        if (isNaN(check[0])) {
...
async function leechWithRefID(refid) {
  ...
  try {
    ...
    let leechUrl =
      "https://www.tff.org/Default.aspx?pageID=531&hakemID=" + refid;
...
async function collectDataForRefresh(browser, page, leechUrl, refid) {
  ...
  try {
    ...
    try {
      ...
      logger.error(
        "Exception in collectDataForRefresh scrape : ",
        err.toString()
      );
    }
  } catch (e) {
...
async function fillRefs() {
  ...
  for (let i = 0; i < refIDs.length; i++) {
    ...
    await Referee.deleteOne({ refID: refIDs[i] })
      .then(function () {
...
async function refreshRefs() {
  ...
  for (let i = 0; i < refIDs.length; i++) {
    ...
    if (newMatches.length != 0) {
      let arr = await Referee.find({ refID: refIDs[i] }).select(
        "matchesRuled -_id"
      );
...
async function searchBySubstr(search) {
  search = search.trim();
  console.log(search);
  let refs = await Referee.find({ name: new RegExp(search, "i") }).select(
    "name refID -_id"
  );
  console.log(refs);
  return refs;
