...
async function collectData(browser, page, leechUrl) {
  ...
  try {
    ...
    try {
      await page.waitForSelector("td.MacDetayAltBG", {
        //find the cell that holds the details for the game and wait until it is visible
        timeout: 2000,
        state: "visible",
      });

      let homeTeamName = await page
        .locator("//a[contains(@id,'MacBilgiDisplay1_dtMacBilgisi_lnkTakim1')]")
        .elementHandles();
      let awayTeamName = await page
        .locator("//a[contains(@id,'MacBilgiDisplay1_dtMacBilgisi_lnkTakim2')]")
        .elementHandles();
      let homeTeamScore = await page
        .locator(
          "//span[contains(@id,'MacBilgiDisplay1_dtMacBilgisi_lblTakim1Skor')]"
        )
        .elementHandles();
      let awayTeamScore = await page
        .locator(
          "//span[contains(@id,'MacBilgiDisplay1_dtMacBilgisi_Label12')]"
        )
        .elementHandles();
      ...
      let divSpans = await page
        .locator("td.MacDetayAltBG>div span")
        .allTextContents();
      ...
      for (let i = 1; i < refs.length; i++) {
        //starts from 1, as 0 has the name of stadium
      ...
      // //cards
      //home
      let homeCardPlayers = await page
        .locator("//td[not(@class)]/a[contains(@id,'grdTakim1_rptKartlar')]")
        .elementHandles(); //select all the anchor elements that has grdTakim1_rptKartlar in their id attribute and inside a td that does not have class attribute
      let homeCardPlayerTime = await page
        .locator("//td[not(@class)]/span[contains(@id,'grdTakim1_rptKartlar')]")
        .elementHandles(); //select all the span elements that has grdTakim1_rptKartlar in their id attribute and inside a td that does not have class attribute
      let homeCardPlayerCardImg = await page
        .locator("//td[not(@class)]/img[contains(@id,'grdTakim1_rptKartlar')]")
        .elementHandles(); //select all the span elements that has grdTakim1_rptKartlar in their id attribute and inside a td that does not have class attribute
      ...
      for (let i = 0; i < homeCardPlayers.length; i++) {
        homeCards.push({
          ...
          cardType:
            (await homeCardPlayerCardImg[i].getAttribute("src")).indexOf(
              "sarikart"
            ) !== -1
              ? "yellow"
              : "red", //might be wrong as there can be 2 yellow then red
        });
      }

      //away
      let awayCardPlayers = await page
        .locator("//td[not(@class)]/a[contains(@id,'grdTakim2_rptKartlar')]")
        .elementHandles();
      let awayCardPlayerTime = await page
        .locator("//td[not(@class)]/span[contains(@id,'grdTakim2_rptKartlar')]")
        .elementHandles();
      let awayCardPlayerCardImg = await page
        .locator("//td[not(@class)]/img[contains(@id,'grdTakim2_rptKartlar')]")
        .elementHandles();
      ...
      for (let i = 0; i < awayCardPlayers.length; i++) {
        awayCards.push({
          ...
          cardType:
            (await awayCardPlayerCardImg[i].getAttribute("src")).indexOf(
              "sarikart"
            ) !== -1
              ? "yellow"
              : "red", //might be wrong as there can be 2 yellow then red
      ...
      //goals
      //home
      let homeGoalsInfo = await page
        .locator("//td[not(@class)]/a[contains(@id,'grdTakim1_rptGoller')]")
        .elementHandles();
      let awayGoalsInfo = await page
        .locator("//td[not(@class)]/a[contains(@id,'grdTakim2_rptGoller')]")
        .elementHandles();
  ...
  let data = {
    ...
    HomeCards: homeCards,
    AwayCards: awayCards,
    HomeGoalsDetails: homeGoals,
    AwayGoalsDetails: awayGoals,
    MatchID: "",
  };
  return data;
}
...
async function leechWithMatchID(matchid) {
  ...
  try {
    ...
    let leechUrl =
      "https://www.tff.org/Default.aspx?pageID=29&macId=" + matchid;
...
async function matchesToDB() {
  for (let i = 0; i < 342 /*match code*/; i++) {
    ...
    //scrape it again
    mongoose.connect(
      "mongodb+srv://cs308team18:BestTeamThereEverWasTeam18@tff-ras.q9epijv.mongodb.net/?retryWrites=true&w=majority"
    );
    let match = await leechWithMatchID(search.toString());
    console.log(match);
    await Fixture.deleteOne({ MatchID: search.toString() })
      .then(function () {
...
async function collectDataForDate(browser, page, leechUrl, date) {
  ...
  try {
    ...
    try {
      ...
      await page
        .locator("//input[contains(@id, 'dateBaslangic_dateInput_TextBox')]")
        .fill(date);
      await page
        .locator("//input[contains(@id, 'dateBitis_dateInput_TextBox')]")
        .fill(date);
      ...
      let matches = await page
        .locator("table.MasterTable_TFF_Contents >tbody tr td a")
        .allTextContents();
...
async function searchBySubstr(search) {
  ...
  let Matches;
  try {
    Matches = await Fixture.find({
      $and: [
        { "Time.date": { $gte: todayString } },
        {
          $or: [
            { "Teams.home": new RegExp(search, "i") },
            { "Teams.away": new RegExp(search, "i") },
          ],
        },
      ],
    });
